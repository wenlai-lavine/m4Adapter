...
model.add_adapter(adapter_task_name, config=adapter_config)
...
p = np.array(
    [len(train_dataloaders[y]) * 1.0 / sum([len(train_dataloaders[x]) for x in list_of_tasks]) for y in list_of_tasks])
...
steps = args.meta_epochs * sum([len(train_dataloaders[x]) for x in list_of_tasks]) // (
            args.task_per_queue * args.update_step)
...
def UniformBatchSampler():
    p = np.array([len(train_dataloaders[y]) * 1.0 / sum([len(train_dataloaders[x]) for x in list_of_tasks]) for y in
                  list_of_tasks])
    p_temp = np.power(p, 1.0 / args.temp)
    p_temp = p_temp / np.sum(p_temp)
    ...
    try:
        for j, metabatch in enumerate(sampler):
            ...
            if (j + 1) % args.log_interval == 0:
                ...
                time.time() - global_time, j + 1, total_loss, val_loss_total))
                print('===============================================')
                global_time = time.time()
                ...
                if avg_all_losses < min_task_losses:
                    ...
                    if dist.get_rank() == 0:
                        model.module.save_adapter(os.path.join(args.save, "save_adapter"), adapter_task_name)
                        print("Saving Model ... ...")
