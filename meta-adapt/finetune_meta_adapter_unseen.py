...
from finetune_data import CorpusM2M100
...
parser.add_argument('--meta_task', type=str, default='EUbookshop,KDE4,OpenSubtitles,QED,TED2020,Ubuntu')
parser.add_argument('--data_path', type=str, default='')
parser.add_argument('--adapter_type', type=str, default='pfeiffer')
parser.add_argument('--adapter_path', type=str, default='pfeiffer')
...
list_of_tasks.append(args.meta_task)
...
for k in list_of_tasks:
	...
	train_data_file = os.path.join(args.data_path, task_list[1], 'valid.tsv')
	valid_data_file = os.path.join(args.data_path, task_list[1], 'test.tsv')
...
adapter_task_name = 'meta-agnostic-adapter'
model.load_adapter(
        os.path.join(args.adapter_path),
        config=args.adapter_type,
        load_as=adapter_task_name,
    )
model.train_adapter([adapter_task_name])
model.set_active_adapters([adapter_task_name])
model.cuda()
...
optimizer = AdamW(optimizer_grouped_parameters, lr=args.meta_lr, eps=args.adam_epsilon)
...
for k in list_of_tasks:
	...
	train_sampler[k] = DistributedSampler(train_corpus[k].datasets)

	train_dataloaders[k] = DataLoader(
		train_corpus[k].datasets,
		...
		for j,metabatch in enumerate(sampler):
			...
			if (j + 1) % args.log_interval == 0:
				...
				if avg_all_losses < min_task_losses:
					...
					model.module.save_adapter(args.save, adapter_task_name)
					print("Saving Model ... ...")
