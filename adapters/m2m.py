...
class M2M100SelfAttentionAdaptersModule(AdapterLayerBaseMixin, nn.Module):
    ...
    def transformer_layer_norm(self):
        # M2M has layer norms before each component
        if self.config.model_type == "m2m_100":
            return None
        else:
            return self.parent.self_attn_layer_norm


class M2M100CrossAttentionAdaptersModule(AdapterLayerBaseMixin, nn.Module):
    def __init__(self, parent):
        super().__init__()
        # keep a reference to the parent module without registering as a submodule
        object.__setattr__(self, "parent", parent)
...
class M2M100OutputAdaptersModule(AdapterLayerBaseMixin, nn.Module):
    def __init__(self, parent):
        super().__init__()
        # keep a reference to the parent module without registering as a submodule
        object.__setattr__(self, "parent", parent)
...
class M2M100EncoderLayerAdaptersMixin:
    """Adds adapters to the M2M100EncoderLayer module of M2M."""

    def _init_adapter_modules(self):
        self.attention_adapters = M2M100SelfAttentionAdaptersModule(self)
        self.output_adapters = M2M100OutputAdaptersModule(self)
...
class M2M100DecoderLayerAdaptersMixin(M2M100EncoderLayerAdaptersMixin):
    ...
    def _init_adapter_modules(self):
        ...
        self.cross_attention_adapters = M2M100CrossAttentionAdaptersModule(self)
...
class M2M100EncoderDecoderAdaptersMixin:
    """Adds adapters to the M2M100Encoder or M2M100Decoder module."""
...
class M2M100ModelAdaptersMixin(ModelAdaptersMixin):
    ...
    def _init_adapter_modules(self):
        ...
        if hasattr(self, "encoder"):
            # In M2M, the invertible adapters are implemented by the encoder module.
...
class M2M100ModelHeadsMixin(ModelWithFlexibleHeadsAdaptersMixin):
    """
    Adds flexible heads to a M2M model.
    """
